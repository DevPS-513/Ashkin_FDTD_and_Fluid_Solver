function [ Tx ,t1,t2,t3,t4,t5,t6] = Calculate_Tx_AB_at_Ey_v2( i,j,Ex,Ey,Dx,Dy,Hz,Hz_n_prev,Bz,Bz_n_prev,dx,dy )
...
%t1=(1/(dx)).*(Dx_av(i,j).*Ex_av(i,j)-Dx_av(i-1,j).*Ex(i-1,j));
t1=0;
% t2=\px DyEy

Dy2=(1/2)*(Dy(i,j)+Dy(i+1,j));
Dy1=(1/2)*(Dy(i,j)+Dy(i-1,j));
Ey2=(1/2)*(Ey(i,j)+Ey(i+1,j));
Ey1=(1/2)*(Ey(i,j)+Ey(i-1,j));

%t2=(1/(2*dx)).*(Dy(i+1,j).*Ey(i+1,j)-Dy(i-1,j).*Ey(i-1,j));

    t2=(1/(1*dx)).*(Dy2.*Ey2-Dy1.*Ey1);

% t3=\px(BzHz) ( double diff for now? )
    % t3=(1/(3*dx)).*(Bz_av(i+1,j).*Hz_av(i+1,j)-Bz_av(i-2,j).*Hz_av(i-2,j));
    t3=(1/(dx)).*(Bz_av(i,j).*Hz_av(i,j)-Bz_av(i-1,j).*Hz_av(i-1,j));

% t4=2*DxEx
%     t4=(2/(3*dx)).*(Dx_av(i+1,j).*Ex_av(i+1,j)-Dx_av(i-2,j).*Ex(i-2,j));
    t4=(1/(1*dx)).*(Dx_av(i,j).*Ex_av(i,j)-Dx_av(i-1,j).*Ex_av(i-1,j));
