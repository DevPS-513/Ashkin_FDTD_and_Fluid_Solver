...
function [fx_EM,fy_EM,eta_EM,Hz_EM,Sy_avg,fx_avg,fy_avg]=casner_EM_code_interp(x_CFD,y_CFD,r,er_low,er_high,LAMBDA,dx,beam_width,x_size,y_size,a,sim_cycles,model,plot_on,source_direction,sos)
...
%       Hz(source1_x,nyg1:nyg2)=gauss_y(nyg1:nyg2).*a/eta_o.*cos(2*pi*f*(t(n)))+ Hz(source1_x,nyg1:nyg2);            % X- Guassian %         
    eta_r=sqrt(mr(round(Nx/2),source1_y)./er(round(Nx/2),source1_y));
        Hz(nxg1:nxg2,source1_y)=a_o*gauss_x(nxg1:nxg2)'.*(a/(2*eta_o)).*sin(2*pi*f*(t(n)))+ Hz(nxg1:nxg2,source1_y);            % X- Guassian %         
...
Sy_avg(1,n)=mean(Sy_flux(1:n));
    ...
    [Ty ] = Calculate_Ty_EL( i,j,Ex,Ey,Dx,Dy,Hz,Hz_n_prev,Bz,Bz_n_prev,dx,dy );           % 
        [Tx,t1,t2,t3,t4,t5] = Calculate_Tx_EL(i,j,Ex,Ey,Dx,Dy,Hz,Hz_n_prev,Bz,Bz_n_prev,dx,dy );   
...
Sy_avg(1,n)=mean(Sy_flux(1:n));
            ...
            [Tx,t1,t2,t3,t4] = Calculate_Tx_AMP(i,j,Ex,Ey,Dx,Dy,Hz,Hz_n_prev,Bz,Bz_n_prev,dx,dy );  
